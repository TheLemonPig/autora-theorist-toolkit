...
class HybridModel(BaseEstimator):
    ...
    def __init__(self, trees=3, sklearn_regressor=LinearRegression):
        ...
        self.tree_sets = [[SymbolicRegressor(temperature, prior_dict_) for temperature in temperatures_] for _ in
                          range(trees)]
    ...
    def fit_step(self, tree_set, X, y):
        for tree, i in enumerate(tree_set):
            ...
            if not tree.visited():
                ...
                self.reg.fit(W, y)
                mdl = minimum_description_length(y_true=y, y_pred=self.reg.predict(W),
                                                 n=X.shape[0], k=len(tree.get_parameters()),
                                                 prior_dict=self.prior_dict, expr_str=self.make_full_tree_str(),
    ...
    def tree_swap(self, tree_set, X, y):
        ...
        mdl1 = minimum_description_length(y_true=y, y_pred=self.reg.predict(W),
                                          n=X.shape[0], k=len(tree_set[j].get_parameters()),
                                          prior_dict=self.prior_dict, expr_str=self.make_full_tree_str(),
        ...
        W = self.intermediate(X)
        self.reg.fit(W, y)
        mdl2 = minimum_description_length(y_true=y, y_pred=self.reg.predict(W),
                                          n=X.shape[0], k=len(tree_set[j + 1].get_parameters()),
                                          prior_dict=self.prior_dict, expr_str=self.make_full_tree_str(),
                                          temperature=tree_set[j + 1].temperature)
        self.swap(tree_set, 0, j + 1)
        mdl_change = mdl1 * (1 / tree_set[j + 1].temperature - 1 / tree_set[j].temperature) + mdl2 * (
                    1 / tree_set[j].temperature - 1 / tree_set[j + 1].temperature)
